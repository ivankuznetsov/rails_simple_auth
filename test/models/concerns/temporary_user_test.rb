...
class TemporaryUserTest < Minitest::Test
  ...
  def test_temporary_scope
    ...
    User.create!(email_address: 'perm@example.com', password: 'password123', temporary: false)

    assert_equal [temp_user], User.temporary.to_a
  end

  def test_permanent_scope
    User.create!(email_address: 'temp@example.com', password: 'password123', temporary: true)
    perm_user = User.create!(email_address: 'perm@example.com', password: 'password123', temporary: false)
  ...
  def test_convert_to_permanent_updates_all_fields
    ...
    assert_equal 'permanent@example.com', user.email_address
  ...
  def test_convert_to_permanent_fails_when_email_taken_by_permanent_user
    User.create!(email_address: 'taken@example.com', password: 'password123', temporary: false)
    temp_user = User.create!(email_address: 'temp@example.com', password: 'password123', temporary: true)
  ...
  def test_convert_to_permanent_allows_email_from_another_temporary_user
    User.create!(email_address: 'other_temp@example.com', password: 'password123', temporary: true)
    temp_user = User.create!(email_address: 'temp@example.com', password: 'password123', temporary: true)
